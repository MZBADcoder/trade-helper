...
async def test_demo_replay_window_is_stable_and_fixed_length() -> None:
    service = _build_demo_service(
        now=datetime(2026, 2, 27, 14, 0, 0, tzinfo=timezone.utc),
        today=date(2026, 2, 27),
    )

    first = await service.replay_window()
    second = await service.replay_window()
...
async def test_demo_bars_reject_non_amd_ticker() -> None:
    service = _build_demo_service(
        now=datetime(2026, 2, 27, 14, 0, 0, tzinfo=timezone.utc),
        today=date(2026, 2, 27),
    )
    ...
    with pytest.raises(ValueError, match="AMD"):
        await service.list_bars_with_meta(
            ticker="AAPL",
            timespan="minute",
            multiplier=1,
        )
...
async def test_demo_stream_events_include_supported_market_messages() -> None:
    service = _build_demo_service(
        now=datetime(2026, 2, 27, 14, 0, 0, tzinfo=timezone.utc),
        today=date(2026, 2, 27),
    )

    events = await service.stream_events(step=7, channels={"quote", "trade", "aggregate"})
