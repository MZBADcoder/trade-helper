...
async def test_add_item_normalizes_ticker_and_user_scope() -> None:
    repo = FakeWatchlistRepository()
    service = WatchlistApplicationService(uow=FakeUoW(watchlist_repo=repo))

    created = await service.add_item(user_id=7, ticker=" aapl ")

    assert created.ticker == "AAPL"
    assert repo.last_add_call == (7, "AAPL")


async def test_list_items_rejects_invalid_user_id() -> None:
    repo = FakeWatchlistRepository()
    service = WatchlistApplicationService(uow=FakeUoW(watchlist_repo=repo))

    with pytest.raises(ValueError, match="Invalid user id"):
        await service.list_items(user_id=0)


async def test_remove_item_normalizes_ticker() -> None:
    repo = FakeWatchlistRepository()
    repo.items_by_user[3] = [WatchlistItem(ticker="MSFT", created_at=None)]
    service = WatchlistApplicationService(uow=FakeUoW(watchlist_repo=repo))

    await service.remove_item(user_id=3, ticker=" msft ")
