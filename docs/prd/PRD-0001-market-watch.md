...
> 状态：执行中（阶段收敛：Stock 主线优先，Options HOLD）  
> 最后更新：2026-03-01  
> 负责人：____
...
## 1.1 当前执行策略（2026-02-16）

- 本阶段仅推进 **Stock 数据主线**：watchlist、snapshots、bars、实时状态与降级链路。
- 期权相关开发进入 HOLD：后端 options 相关实现/扩展测试、前端 options 相关实现/E2E。
- 恢复条件：Stock 数据主线完成并达到里程碑验收后，再恢复期权开发。
- 原因：股票与期权并行推进会显著增加联调和状态机复杂度，影响迭代稳定性与交付节奏。

## 2. 背景与动机

- 现状/痛点：
  - 1) 前端已经有 Terminal 原型，但此前 `/demo` 的数据来源是前端本地模拟；`/terminal` 虽已有基础 bars 接口映射，但仍缺少“真正实时”的数据链路与期权观察能力。
...
5) 用户切换到 options 视图：系统加载到期列表与期权链快照；用户点击某个合约后进入合约详情，并开始订阅该合约实时更新。（HOLD，下一阶段恢复）  
...
- S1：期权链筛选与视图优化（DTE、行权价范围、moneyness/ATM，HOLD）
- S2：合约收藏（将合约加入关注列表，快速回访，HOLD）
- S3：服务端指标计算（保证多端公式一致；前端仍可作为兜底）
- S4：历史期权 bars（按选中合约查询，并支持基础图表，HOLD）
...
- 路由约定：
  - `/demo`：保持独立 demo，但改为**后端 mock 回放入口**；免登录，不依赖 Massive，固定回放最近一个已完成交易日的 `AMD` 盘中 30 分钟窗口（REST 首屏 + WS 循环推送）
...
- [ ] `/demo` 免登录可用：固定展示 `AMD`，首屏返回后端 mock 的 30 分钟盘中窗口 bars，且 WS 可循环回放该窗口
- [ ] 期权链可用：能按 underlying + expiration 加载 chain，并展示最小字段集（HOLD，下一阶段验收）
- [ ] 选中 1 个期权合约后，合约详情能够持续更新（WS 或 polling）（HOLD，下一阶段验收）
