# PRD-0002 — IV Percentile 告警（Polygon.io / Massive.com，美股期权）
...
- 我们要做的是：**一个交易助手，通过 `polygon.io` 的 API 获取美股期权数据，持续追踪关注标的的期权隐含波动率（IV），当 IV 偏高时发出提示/推送，用于辅助交易决策。**
...
- G1：能够通过 `polygon.io` 拉取期权 IV 相关数据，并对关注列表进行自动监控。
...
1) 用户配置 `polygon.io` API Key、关注标的列表（watchlist）、监控规则  
2) 系统按设定频率从 `polygon.io` 拉取相关期权数据（期权链/合约 IV 或聚合后的代表性 IV）  
...
- Worker（离线计算）：Celery Workers
  - 负责：拉取 `polygon.io` 数据、计算 Percentile/Rank、生成告警、发送飞书/Email、写入数据库
  - 能力：并发、超时、重试、失败隔离
...
- 性能：刷新频率默认 15 分钟（与当前 `polygon.io` 15 分钟延迟套餐匹配）；并关注请求量与成本
- 可靠性：API 失败要可见（错误提示/重试/退避），并避免错误导致持续刷通知
- 安全：
  - `polygon.io` API Key 需安全存储（避免明文散落），最小权限与可一键替换
...
- MVP 验收清单：
  - [ ] 能配置 `polygon.io` API Key，并成功拉取至少 1 个标的的期权 IV 相关数据
...
9) `polygon.io`：$29 计划；15 分钟延迟；当前认为无 API 限制（仍需以官方条款为准）  
